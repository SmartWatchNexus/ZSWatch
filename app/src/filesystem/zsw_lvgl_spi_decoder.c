                                ...
                                uint32_t *br)
{
    ...
    if ((read_address != orig_read_address) && btr < sizeof(fix_alignment_buffer)) {
        extra_bytes = orig_read_address - read_address;
    } else {
        extra_bytes = 0;
        read_address = orig_read_address;
    }

    rc = flash_area_read(flash_area, read_address, extra_bytes ? fix_alignment_buffer : buf, btr + extra_bytes);
    if (rc != 0) {
    ...
    if (extra_bytes) {
        memcpy(buf, fix_alignment_buffer + extra_bytes, btr);
    }
